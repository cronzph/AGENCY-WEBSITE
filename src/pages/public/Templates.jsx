...
const Templates = () => {
  ...
  const openInquiry = (template) => {
    ...
    if (template.status === 'coming') {
      setPreInquiryOpen(true);
    } else {
      setModalOpen(true);
    }
  };
                ...
                >
                  ...
                  <div className="p-4 sm:p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2.5">
                      ...
                      {/* Inquire / Notify Me button — full width on mobile */}
                      {template.status === 'coming' ? (
                        <button
                          onClick={() => openInquiry(template)}
                          className="w-full sm:w-auto sm:px-5 py-2.5 rounded-xl font-bold text-white text-sm transition-all hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-500"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
